...
- [matter]: Add the (optional) `TimeSynchronization` cluster to the root endpoint of every Matterbridge server node, reporting `UtcTime`/`Granularity` from the host system clock via the new `MatterbridgeTimeSynchronizationServer` behavior.
...
- [logger]: The `node-ansi-logger` package integrates now a chainable ANSI tagged template styling API. See matterbridge-mqtt plugin for an example.
